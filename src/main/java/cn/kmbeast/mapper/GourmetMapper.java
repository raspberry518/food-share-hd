...
public interface GourmetMapper {
    ...
    List<GourmetListVO> queryList(GourmetQueryDto gourmetQueryDto);

    Integer queryCount(GourmetQueryDto gourmetQueryDto);

    List<GourmetVO> queryDetail(GourmetQueryDto gourmetQueryDto);

    List<GourmetListVO> queryByIds(@Param(value = "ids") List<Integer> ids);

    List<GourmetListVO> queryByView(GourmetQueryDto gourmetQueryDto);
}