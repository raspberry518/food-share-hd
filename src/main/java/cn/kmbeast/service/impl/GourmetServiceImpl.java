...
//        List<GourmetVO> gourmetList = gourmetMapper.query(gourmetQueryDto);
        List<GourmetListVO> gourmetListVOS = gourmetMapper.queryList(gourmetQueryDto);
        Integer totalCount = gourmetMapper.queryCount(gourmetQueryDto);
        for (GourmetListVO gourmetListVO : gourmetListVOS) {
            gourmetListVO.setDetail(TextUtil.parseText(gourmetListVO.getDetail(), 170));
        }
        //将gourmetList封装为gourmetListVOS，展示列表需求
        /*List<GourmetListVO> gourmetListVOS = gourmetList.stream()
    ...
    public Result<List<ChartVO>> daysQuery(Integer day) {
        ...
        List<LocalDateTime> localDateTimes = gourmetVOS.stream()
     ...
     */
    @Override
    public Result<List<GourmetListVO>> queryByView(GourmetQueryDto gourmetQueryDto) {
        List<GourmetListVO> gourmetVOS = gourmetMapper.queryByView(gourmetQueryDto);
        return ApiResult.success(gourmetVOS);
    }
